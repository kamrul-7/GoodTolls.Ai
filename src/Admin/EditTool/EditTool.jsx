...
const EditTool = () => {
    ...
    const [day, month, year] = toolData.date.split('/');
    ...
    useEffect(() => {
        const contentBlock = htmlToDraft(toolData.description)
    ...
    useEffect(() => {
        const contentBlock = htmlToDraft(toolData.works)
    ...
    const [toolNameState, setToolNameState] = useState(toolData.toolName)
    const [toolLink, setToolLink] = useState(toolData.link)
    const [toolPrice, setToolPrice] = useState(toolData.price)
    const [toolFacebook, setToolFacebook] = useState(toolData.facebook)
    const [toolTwitter, setToolTwitter] = useState(toolData.twitter)
    const [toolLinkedin, setToolLinkedin] = useState(toolData.linkedin)
    const [toolDiscord, setToolDiscord] = useState(toolData.discord)
    const [toolVerify, setToolVerify] = useState(toolData.verified)
    const [toolPriceType, setToolPriceType] = useState(toolData.priceType)
    const [toolPriceDuration, setToolPriceDuration] = useState(toolData.pricePeriod)

    useEffect(() => {
        setSelected(toolData.SubCategory)
    }, [])

    const handleNameChange = (event) => {
        setToolNameState(event.target.value)
    }
    ...
    const handleFacebookChange = (event) => {
        setToolFacebook(event.target.value)
    }

    const handleTwitterChange = (event) => {
        setToolTwitter(event.target.value)
    }

    const handleLinkedinChange = (event) => {
        setToolLinkedin(event.target.value)
    }
    const handleDiscordChange = (event) => {
        setToolDiscord(event.target.value)
    }
    const handleVerifyChange = (event) => {
        setToolVerify(!toolVerify)
    }

    const handleToolPriceTypeChange = (event) => {
        setToolPriceType(event.target.value)
    }
    const handleToolPriceDurationChange = (event) => {
        setToolPriceDuration(event.target.value)
    }
    ...
    const handleSubmit = (event) => {
        ...
        if (toolNameState.length != 0 && selected.length != 0 && toolLink.length != 0 && imageId != null
            ...
            formdata.append('SubCategory', selected)
            formdata.append('link', toolLink)
            if (file != null) {
                formdata.append('image', file)
            }
            formdata.append('imageId', imageId)
            formdata.append('priceType', toolPriceType)
            formdata.append('price', toolPrice)
            formdata.append('pricePeriod', toolPriceDuration)
            formdata.append('date', date)
            formdata.append('facebook', toolFacebook)
            formdata.append('twitter', toolTwitter)
            formdata.append('linkedin', toolLinkedin)
            formdata.append('discord', toolDiscord)
            formdata.append('description', description)
            formdata.append('works', works)
            formdata.append('toolId', toolData._id)
            formdata.append('descriptionPath', toolData.descriptionPath)
            formdata.append('usagesPath', toolData.usagesPath)
            setIsLoading(true)
    ...
    return (
        <div>
            {
                isLoading ?
                    <span className="loading loading-ring md:w-40 md:h-40 w-20 h-20 md:ml-[35%] my-[25%]"></span>
                    :
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className='w-full flex pb-5 mb-5 border-b border-[#EAECF0]'>
                                ...
                                <div className='py-[10px] px-[14px] border rounded-lg h-[44px] w-[512px]'>
                                    <input onChange={handleNameChange} value={toolNameState} name='toolName' placeholder="Chapple" className=' h-full my-auto w-full focus:outline-0 text-base text-[#101828] border-0 focus:ring-0' type="text" />
                                </div>
                            </div>
                            ...
                            <div className='w-full flex pb-5 mb-5 border-b border-[#EAECF0]'>
                                ...
                                <div>
                                    <label className="relative inline-flex items-center mr-5 cursor-pointer">
                                        <input checked={toolVerify} onChange={handleVerifyChange} type="checkbox" name="verify" className="sr-only peer" />
                                ...
                                <div className="w-[400px] border px-[14px] py-[10px] flex flex-wrap gap-2 border-[#D0D5DD] rounded-lg" tabIndex={0} onFocus={() => { inputRef.current.focus() }}>
                                    ...
                                    <div onBlur={setBlur} className={`w-6/12`}>
                                        <div className="relative">
                                            <input placeholder='Choose Sub Category' ref={inputRef} onFocus={setFocus} className={`w-full h-6 focus:border-0 focus:outline-0 border-0 focus:ring-0`} onKeyUp={handleKeyPress} type="text" />
                            ...
                            <div className='w-full flex pb-5 mb-5 border-b border-[#EAECF0]'>
                                ...
                                <div className='py-[10px] px-[14px] border rounded-lg h-[44px] w-[512px] flex items-center'>
                                    ...
                                    <input onChange={handleLinkChange} value={toolLink} name='link' placeholder="www.chapple.ai" className='border-0 focus:ring-0 ml-2 mb-[1px] h-full my-auto w-full focus:outline-0 text-base text-[#101828]' type="url" />
                                </div>
                            </div>
                            ...
                            <div className='w-full flex pb-5 mb-5 border-b border-[#EAECF0]'>
                                ...
                                <div className=''>
                                    ...
                                    <div className='mb-4 py-[10px] px-[14px] border rounded-lg h-[44px] w-[512px]' >
                                        <select value={toolPriceType} onChange={handleToolPriceTypeChange}
                                            name='priceType' className='-mt-[10px] border-0 focus:ring-0  w-full focus:outline-0 text-base font-medium'>
                                    ...
                                    <div className='py-[10px] px-[14px] border rounded-lg h-[44px] w-[512px] flex justify-between items-center ' >
                                        ...
                                        <div className='w-full flex items-center'>
                                            ...
                                            <input onChange={handlePriceChange} value={toolPrice} name='price' placeholder="Amount" className='border-0 focus:ring-0 ml-2 mb-[1px] h-full w-full focus:outline-0 text-base text-[#101828]' type="text" />
                                        </div>

                                        {/* Price duration */}
                                        <select value={toolPriceDuration} onChange={handleToolPriceDurationChange} name='pricePeriod' className=' border-0 focus:ring-0 w-3/12 ml-[14px] focus:outline-0 text-base'>
                            ...
                            <div className='w-full flex pb-5 mb-5 border-b border-[#EAECF0]'>
                                ...
                                <div className='py-[10px] px-[14px] border rounded-lg h-[44px] w-[512px] flex items-center'>
                                    ...
                                    <DatePicker
                                        ...
                                        className="focus:outline-0 w-[480px] h-[33px] border-0 focus:ring-0"
                                        isClearable
                                        showIcon
                                ...
                                <div>
                                    <div className='mb-4 border rounded-lg h-[44px] w-[512px] flex'>
                                        ...
                                        <div className='py-[10px] px-[14px]'>
                                            <input onChange={handleFacebookChange} value={toolFacebook} name='facebook' placeholder="Chapple" className='border-0 focus:ring-0 h-full my-auto w-full focus:outline-0 text-base text-[#101828]' type="text" />
                                        </div>
                                    </div>
                                    <div className='mb-4 border rounded-lg h-[44px] w-[512px] flex'>
                                        <div className='w-3/12 py-[10px] px-[14px] border-r text-[#475467] text-base'>
                                        ...
                                        <div className='py-[10px] px-[14px]'>
                                            <input onChange={handleLinkedinChange} value={toolLinkedin} name='linkedin' placeholder="Chapple" className='border-0 focus:ring-0 h-full my-auto w-full focus:outline-0 text-base text-[#101828]' type="text" />
                                        </div>
                                    </div>
                                    <div className='mb-4 border rounded-lg h-[44px] w-[512px] flex'>
                                        <div className='w-3/12 py-[10px] px-[14px] border-r text-[#475467] text-base'>
                                        ...
                                        <div className='py-[10px] px-[14px]'>
                                            <input onChange={handleTwitterChange} value={toolTwitter} name='twitter' placeholder="Chapple" className='border-0 focus:ring-0 h-full my-auto w-full focus:outline-0 text-base text-[#101828]' type="text" />
                                        </div>
                                    </div>
                                    <div className=' border rounded-lg h-[44px] w-[512px] flex'>
                                        ...
                                        <div className='py-[10px] px-[14px]'>
                                            <input onChange={handleDiscordChange} value={toolDiscord} name='discord' placeholder="Chapple" className='border-0 focus:ring-0 h-full my-auto w-full focus:outline-0 text-base text-[#101828]' type="text" />
                                        </div>
                                    </div>
                                </div>
                            </div>
